...
async function migrateTools() {
  ...
  try {
    ...
    for (const tool of tools) {
      try {
        ...
        batch.set(toolRef, {
          ...
          createdAt: admin.firestore.Timestamp.fromDate(new Date(tool.created_at)),
          updatedAt: admin.firestore.Timestamp.fromDate(new Date(tool.updated_at)),
        });
    ...
    console.log(`✅ Tools: ${stats.tools.migrated}/${stats.tools.total} migrated`);
  } catch (error) {
...
async function migrateArticles() {
  ...
  try {
    ...
    for (const article of articles) {
      try {
        ...
        batch.set(articleRef, {
          ...
          publishedAt: admin.firestore.Timestamp.fromDate(new Date(article.published_at)),
          readTime: article.read_time,
          tags: article.tags || [],
          createdAt: admin.firestore.Timestamp.fromDate(new Date(article.created_at)),
        });
    ...
    console.log(`✅ Articles: ${stats.articles.migrated}/${stats.articles.total} migrated`);
  } catch (error) {
...
async function migrateFavorites() {
  ...
  try {
    ...
    for (const favorite of favorites) {
      try {
        ...
        batch.set(favoriteRef, {
          ...
          createdAt: admin.firestore.Timestamp.fromDate(new Date(favorite.created_at)),
        });
    ...
    console.log(`✅ Favorites: ${stats.favorites.migrated}/${stats.favorites.total} migrated`);
  } catch (error) {
...
async function migrateReviews() {
  ...
  try {
    ...
    console.log(`✅ Reviews: ${stats.reviews.migrated}/${stats.reviews.total} migrated`);
  } catch (error) {
...
async function migrateSearchHistory() {
  ...
  try {
    ...
    for (const search of searchHistory) {
      try {
        ...
        batch.set(searchRef, {
          ...
          createdAt: admin.firestore.Timestamp.fromDate(new Date(search.created_at)),
        });
    ...
    if (count > 0) {
      await batch.commit();
      console.log(`  ✓ Committed final batch of ${count} search history entries`);
    }
    
    console.log(`✅ Search History: ${stats.searchHistory.migrated}/${stats.searchHistory.total} migrated`);
  } catch (error) {
    console.error("❌ Error migrating search history (table may not exist):", error.message);
...
async function migrateAnalyticsEvents() {
  ...
  try {
    const events = await sql`SELECT * FROM analytics_events ORDER BY id LIMIT 10000`;
    ...
    console.log(`✅ Analytics Events: ${stats.analyticsEvents.migrated}/${stats.analyticsEvents.total} migrated`);
  } catch (error) {
    console.error("❌ Error migrating analytics events (table may not exist):", error.message);
    // Non-fatal - table might not exist
  }
}
...
async function migrateContactMessages() {
  ...
  try {
    ...
    for (const message of messages) {
      try {
        const messageRef = db.collection("contactMessages").doc(String(message.id));
        batch.set(messageRef, {
          ...
          createdAt: admin.firestore.Timestamp.fromDate(new Date(message.created_at)),
        });
        ...
        console.error(`  ❌ Error migrating contact message ${message.id}:`, error);
    ...
    console.log(`✅ Contact Messages: ${stats.contactMessages.migrated}/${stats.contactMessages.total} migrated`);
  } catch (error) {
    console.error("❌ Error migrating contact messages (table may not exist):", error.message);
    // Non-fatal - table might not exist
  }
}
...
async function setAdminClaim(adminEmail: string) {
  ...
  try {
    ...
    if (usersSnapshot.empty) {
      ...
      console.log("   You'll need to set admin claim after the user logs in for the first time.");
      return;
    }
...
async function migrate() {
  ...
  try {
    ...
    // await setAdminClaim("your-email@gmail.com"); // Uncomment and set your email
    ...
    console.log(`Tools:            ${stats.tools.migrated}/${stats.tools.total} (${stats.tools.errors} errors)`);
    console.log(`Articles:         ${stats.articles.migrated}/${stats.articles.total} (${stats.articles.errors} errors)`);
    console.log(`Favorites:        ${stats.favorites.migrated}/${stats.favorites.total} (${stats.favorites.errors} errors)`);
    console.log(`Reviews:          ${stats.reviews.migrated}/${stats.reviews.total} (${stats.reviews.errors} errors)`);
    console.log(`Search History:   ${stats.searchHistory.migrated}/${stats.searchHistory.total} (${stats.searchHistory.errors} errors)`);
    console.log(`Analytics Events: ${stats.analyticsEvents.migrated}/${stats.analyticsEvents.total} (${stats.analyticsEvents.errors} errors)`);
    console.log(`Contact Messages: ${stats.contactMessages.migrated}/${stats.contactMessages.total} (${stats.contactMessages.errors} errors)`);
    console.log("=".repeat(60));
